...
MongoDataTable.prototype.get = function get(collectionName, options, onDataReady) {
  ...
  function getAndSortData(callback) {
    ...
    collection = collection.aggregate([...searchCriteria, ...columns]);

    if (parseInt(options.length) > 0) {
        collection = collection.aggregate([...searchCriteria, ...columns, {$skip: parseInt(options.start)}, {$limit: parseInt(options.length)}]);
