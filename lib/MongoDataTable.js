...
MongoDataTable.prototype.get = function get(collectionName, options, onDataReady) {
  ...
  function getAndSortData(callback) {
    ...
    collection = collection.aggregate([{$match: searchCriteria}, {$match: columns}]);

    if (parseInt(options.length) > 0) {
        collection = collection.aggregate([{$match: searchCriteria}, {$match: columns}, {$skip: parseInt(options.start)}, {$limit: parseInt(options.length)}]);
